...
var RealEstate = artifacts.require("./RealEstate.sol");     //RealEstate 컨트랙트를 변수로 가져와서 접근가능하게 한다.

contract('RealEstate', function(accounts){      //테스팅할 컨트랙트 이름, accounts 인자를 콜백으로 받는다.
    var realEstateInstance;

    it("컨트랙트의 소유자 초기화 테스팅", function(){   //it를 통해 테스팅할것들 정의
        return RealEstate.deployed().then(function(instance){
            realEstateInstance = instance;
            return realEstateInstance.owner.call();
        }).then(function(owner){
            assert.equal(owner.toUpperCase(), accounts[0].toUpperCase(), "owner 가 가나슈 첫번째 계정과 동일하지 않습니다.");
        });
    });

    it("가나슈 두번째 계정으로 매물 아이디 0번 매힙 후 이벤트 생성 및 매입자 정보와 buyers 배열 테스팅", function(){
        return RealEstate.deployed().then(function(instance){
            realEstateInstance = instance;
